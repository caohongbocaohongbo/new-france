...
class CumulativeDrawdownTest(unittest.TestCase):
    def test_screening_price_history_uses_cumulative_drawdown_from_watch_date(self):
        hist = pd.DataFrame(
            [
        ...
        self.assertEqual(series[-1]["drawdown_pct"], -20.33)
        self.assertNotEqual(series[-1]["drawdown_pct"], -9.47)

    def test_watchlist_current_drawdown_uses_last_cumulative_history_value(self):
        hist = pd.DataFrame(
            [
                {"日期": "2026-05-20", "收盘": 53.25, "涨跌幅": 10.00},
                {"日期": "2026-05-21", "收盘": 52.58, "涨跌幅": -1.26},
    ...
    def test_audit_accepts_reported_cumulative_drawdown_from_price_history(self):
        result = validate_drop_consistency({
            ...
            "drop_pct": -20.33,
            "price_history": [
                {"date": "2026-06-03", "drawdown_pct": -10.86},
                {"date": "2026-06-04", "drawdown_pct": -20.33},
            ],
        })
