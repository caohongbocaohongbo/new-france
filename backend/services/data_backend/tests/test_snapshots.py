...
def test_read_quotes_refetches_when_fresh_local_snapshot_has_degraded_row(monkeypatch, tmp_path):
    monkeypatch.setattr(snapshots, "DATA_BACKEND_DIR", tmp_path)
    monkeypatch.setattr(snapshots, "REPORT_DATA_BACKEND_DIR", tmp_path / "reports")
    monkeypatch.setattr(snapshots, "_MEMORY_CACHE", {})
    monkeypatch.setattr(snapshots, "_now", lambda: datetime(2026, 7, 6, 10, 0, tzinfo=BEIJING_TZ))
    snapshots._write_local_snapshot(
        ...
        {
            "source": "cache",
            "fetched_at": "2026-07-06T09:59:50+08:00",
            "records": [{"代码": "600001", "最新价": 10.0, "source": "tencent", "source_time": None}],
            "received_codes": ["600001"],
        },
    )
    ...
    fetched = pd.DataFrame([{
        ...
        "最新价": 10.0,
        "source": "tencent",
        "source_time": "2026-07-06T09:59:58+08:00",
    }])
    result, meta = snapshots.read_quotes(["600001"], fetcher=lambda codes: fetched)

    assert result["source_time"].tolist() == ["2026-07-06T09:59:58+08:00"]
    assert meta["status"] == "fresh"
    assert meta["source"] == "live"
...
def test_read_quotes_rejects_remote_snapshot_without_requested_coverage(monkeypatch, tmp_path):
    monkeypatch.setattr(snapshots, "DATA_BACKEND_DIR", tmp_path)
    monkeypatch.setattr(snapshots, "REPORT_DATA_BACKEND_DIR", tmp_path / "reports")
    monkeypatch.setattr(snapshots, "_MEMORY_CACHE", {})
    monkeypatch.setattr(snapshots, "_now", lambda: datetime(2026, 7, 6, 10, 0, tzinfo=BEIJING_TZ))
    monkeypatch.setattr(
        ...
        lambda asset: {
            "source": "data-snapshots",
            "fetched_at": "2026-07-06T09:59:40+08:00",
            "records": [{"代码": "600001", "最新价": 10.0}],
            "received_codes": ["600001"],
        },
    )

    result, meta = snapshots.read_quotes(["600002"], fetcher=lambda codes: None)
