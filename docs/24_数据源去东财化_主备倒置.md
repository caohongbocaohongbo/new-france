...
结论：**东财作主力数据源不可持续；新浪（已实测稳定）+ 腾讯 + pytdx + tushare 应为一级源，东财只作末位备用。**
...
| 涨停池 | layer1_data_collector/sources/eastmoney_zt.py | eastmoney → akshare(东财同源) | 未单测（东财封禁中） | ⚠️ 仅 tushare_source.py 可选源；sina 无现成 zt_pool 专有字段 |
...
| # | 文件 | 改动 |
|---|---|---|
| 1 | historical_kline.py | 优先级改为 **sina → eastmoney → akshare**；akshare 移末位（东财同源，封禁时同样死） |
...
| # | 文件 | 改动 |
|---|---|---|
| 4 | 新建 sources/sina_quote.py | 复用已验证 sina bulk ssggzj：全市场实时行情（trade/changeratio/amount/turnover）→ 替代 eastmoney_quote 作 quotes/index_snapshot 主源 |
| 5 | data_backend/snapshots.py + refresh 入口 | quotes/index_snapshot 读源切 sina_quote（东财备用） |
| 6 | eastmoney_zt.py | 涨停池三级：**tushare limit_list_d（主，需 token）→ 东财 zt_pool（备用）→ sina 涨停计算兜底（仅代码清单，无封板/炸板/连板字段，标 degraded）** |
...
### 4.4 涨停池三级（难点，字段不可免费全量替代）
| 级 | 源 | 字段完整度 | 定位 |
|---|---|---|---|
| 1 | tushare limit_list_d | 涨停代码+封板时间等（含字段需 token 权限） | 主（有 token 时） |
| 2 | 东财 zt_pool（现 fetch_zt_pool） | 全字段（封板/首封/炸板/连板） | 备用 |
| 3 | sina bulk 计算涨停（changeratio ≥ 涨停阈值 + 主板/ST/20cm 判定） | 仅代码清单，无封板时间等 | 兜底，标 degraded=true，01/10 依赖封板字段的功能静默跳过 |
...
| 键 | 默认 | 说明 |
|---|---|---|
| KLINE_SOURCE_ORDER | sina,eastmoney,akshare | K线优先级 |
| FUNDFLOW_SOURCE_ORDER | sina,tencent,eastmoney,akshare | 资金流优先级 |
| ZT_SOURCE_ORDER | tushare,eastmoney,sina_calc | 涨停池优先级 |
| TUSHARE_TOKEN | 空 | 空则 tushare 源跳过 |
...
| R3 | tushare 需 token（非全免费） | token 可选，无 token 时走 sina_calc 兜底；主链路不依赖 |
...
| D2 | 涨停池主源 | 有 tushare token → tushare；无 → sina_calc 兜底 + 东财备用 |
