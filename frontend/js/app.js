...
async function loadRecommendations() {
    ...
    try {
        const resp = await apiFetch('/screening/latest?view=compact');
        const data = await resp.json();
...
function buildPriceHistoryFromDetail(detail) {
    ...
    })).filter(row => Number.isFinite(Number(row.close)) && isRealNumber(row.drawdown_pct)).slice(-12);
}
...
function renderRecommendationCharts(stocks) {
    ...
    stocks.forEach(stock => {
        ...
        const rows = (stock.price_history || [])
            .slice(-12)
            .filter(row => Number.isFinite(Number(row.close)) && isRealNumber(row.drawdown_pct));
        if (!rows.length) return;
        const refPrice = toNumber(stock.ref_price, 0);
        const drawVals = rows.map(row => Number(row.drawdown_pct));
        ...
        chart.setOption(mergeChartOption({
            ...
            series: [
                ...
                },
                {
                    type: 'line',
                    name: '回撤%',
                    data: rows.map(row => Number(row.drawdown_pct)),
                    smooth: true,
                    symbol: 'circle',
                    symbolSize: 8,
                    yAxisIndex: 1,
...
async function runScreening() {
    ...
    try {
        ...
        for (let i = 0; i < 100; i++) {
            ...
            const pollResp = await apiFetch('/screening/latest?view=compact', { timeout: 10000 });
